...
for ($x = 0; $x < $msg_num; $x++) {
    ...
    if ($msg_data["from"] == $sender && $msg_data["to"] == $receiver) {

        $user_rs = Database::search("SELECT * FROM `user` WHERE `email`='" . $msg_data["from"] . "' ");
        $user_data = $user_rs->fetch_assoc();

        $img_rs = Database::search("SELECT * FROM `profile_images` INNER JOIN 
        `user` ON `user`.`email` = `profile_images`.`user_email` WHERE `user`.`email`='" . $msg_data["from"] . "' ");
        $img_data = $img_rs->fetch_assoc();

        Database::iud("UPDATE `chat` SET `status`='1' WHERE `from` = '" . $sender . "' ");
...
?>
        ...
        <!-- sender -->
    <?php
    } else if ($msg_data["to"] == $sender && $msg_data["from"] == $receiver) {
    ?>
