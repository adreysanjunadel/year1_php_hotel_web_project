...
<head>
    ...
    <title>Message | Travelpedia </title>
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-12 p-3">
                <h3 class="m-3 fw-bold">My Messages</h3>
                <hr />
                    ...
                    <div class="col-12 col-lg-5 px-0">
                        <div class="bg-white">
                            <div class="bg-light px-4 py-2">
                                ...
                                <div class="col-12">
                                    <h5 class="mb-0 my-1 fw-bold text-dark">Recents</h5>
                                </div>
                                            ...
                                            <div class="message_box" id="message_box">
                                                ...
                                                for ($x = 0; $x < $msg_num; $x++) {
                                                    ...
                                                    if ($msg_data["status"] == 0) {
                                                        ...
                                                        $user_rs = Database::search("SELECT * FROM `admin` WHERE `admin`.`email`='".$msg_data["from"]."' ");
                                                        $user_data = $user_rs->fetch_assoc();

                                                        $img_rs = Database::search("SELECT * FROM `profile_images` WHERE `email`='" . $msg_data["from"] . "' ");
                                                ...
                                                ?>
                                                    ...
                                                    <?php
                                                        ...
                                                        $user_rs = Database::search("SELECT * FROM `user`,`admin` WHERE `user`.`email`='" . $msg_data["from"] . "' OR `admin`.`email` =  '" . $msg_data["from"] . "'");
                                                        $user_data = $user_rs->fetch_assoc();

                                                        $img_rs = Database::search("SELECT * FROM `profile_images` WHERE `user_email`='" . $_SESSION["u"]["email"] . "' ");
                                                        $img_data = $img_rs->fetch_assoc();
                                                    ...
                                                    <div class="list-group rounded-0" onclick="viewMessages('<?php echo ($msg_data['from']); ?>');">
                                                        <a href="#" class="list-group-item list-group-item-action text-black rounded-0 bg-secondary">
                                                            <div class="media">
                                                                ...
                                                                $img_rs2 = Database::search("SELECT * FROM `profile_images` INNER JOIN
                                                                `user` ON `user`.`email` = `profile_images`.`user_email` WHERE `email`='" . $msg_data2["to"] . "' ");
