...
if ($length <= 4 && $length > 0) {
  ...
  for ($x = 0; $x < count($_FILES); $x++){
    if (isset($_FILES["img" . $x])) {
      ...
      if (in_array($file_extension, $allowed_img_extensions)) {
        ...
        $file_name = "../../resources//resort-images//" . $rin . "_" . $x . "_" . uniqid() . $new_img_extension;
        move_uploaded_file($img_file["tmp_name"], $file_name);

        Database::iud("INSERT INTO `resort_images`(`image`,`resort_id`) VALUES ('" . $file_name . "','" . $rin . "') ");
      } else {
  ...
  echo ("Resort Image(s) Saved Successfully");
} else {
