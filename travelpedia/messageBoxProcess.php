...
$selected_mail = $_GET["e"];

$msg_rs = Database::search("SELECT * FROM `chat` WHERE `from`='" . $selected_mail . "' OR `to`= '" .$selected_mail . "' ");
$msg_num = $msg_rs->num_rows;
for ($y = 0; $y < $msg_num; $y++) {
    $msg_data = $msg_rs->fetch_assoc();
    if ($msg_data["from"] == $selected_mail && $msg_data["to"] == $admin) {
...
?>
    ...
    if ($msg_data["to"] == $selected_mail && $msg_data["from"] == $admin) {
